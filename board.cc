...
string Board::getResource(int n) {
    if (n == 0) { return "BRICK";}
...
void Board::update(string & text, int &width, int &pad, int &right_pad, int &left_pad) {
    pad = (width - text.length()) / 2;
    right_pad = pad;
    left_pad = width - text.length() - right_pad;
}

void Board::printTileTop(int prevPad, string v1, string v2, string e) {
    cout << setw(prevPad) << "|" << setw(textPad) << v1 << "|" << "--" << setw(textPad) << e <<  "--" << "|" << setw(textPad) << v2 << "|"; 
}

void Board::printBar(int prevPad) {
    cout << setw(prevPad) << "|" << setw(10) << "|"; 
}

void Board::printTileNum(int prevPad, string e1, string e2, int i) {
...
void Board::printTileResoc(string text, int prevPad, int i) {    
...
void Board::printTileVal(string text, int prevPad, int i) {
...
void Board::printVertex(string i) {
    cout << "|" << setw(textPad) << i << "|"; 
}

void Board::printEdge(int prevPad, string i) {
    cout << setw(textPad + prevPad + 1) << i ; 
}

void Board::printSingleBar(int prevPad) {
...
void Board::printBoard() {
    ...
    printTileTop(spacePad + 1, "0", "0", "1");  // printTileTop(tile[i].getTLVert(), tile[i].getTEdge(), tile[i].getTRVert())
    cout << endl;  
    ...
    // printing 2 tiles 
    spacePad -= 10; 
    printTileTop(spacePad + 1, "2", "3", "3"); 
    printTileVal(tiles[i].getTileValue(), 0, i);
    printTileTop(0, "4", "5", "4"); 
    cout << endl;
    i++;
    printBar(spacePad + textPad); 
    printBar(spacePad - textPad + 1); 
    cout << endl; 
    printTileNum(spacePad, "5", "6", i);
    printTileNum(fillPad, "7", "8", i + 1);
    cout << endl; 
    printTileResoc(tiles[i].getResource(), spacePad + textPad, i);
    printTileResoc(tiles[i + 1].getResource(), fillPad + textPad, i + 1);
    cout << endl; 
    spacePad -= 10;
    printTileTop(spacePad + 1, "6", "7", "9");  
    printTileVal(tiles[i].getTileValue(), 0, i);
    printTileTop(spacePad , "8", "9", "10"); 
    printTileVal(tiles[i].getTileValue(), 0, i + 1);
    printTileTop(spacePad , "10", "11", "11"); 
    cout << endl; 
    printBar(spacePad + textPad);
    ...
    i += 3; 
    printEdge(0, "20");
    printTileNum(spacePad + fillPad, "12", "13", i );
    printTileNum(spacePad + fillPad - 1, "14", "15", i + 1);
    printEdge(fillPad - 1, "16");
    cout << endl; 
    printSingleBar(0); 
    ...
    printSingleBar(fillPad - 1); 
    cout << endl; 
    printTileTop(spacePad + 1, "30", "31", "43");  
    printTileVal(tiles[i].getTileValue(), 0, i);
